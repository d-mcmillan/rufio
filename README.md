...
![Status](https://img.shields.io/badge/status-v1.0.6.3-success)
...
https://github.com/user-attachments/assets/7244a721-89cb-4f44-8f8a-365bd800de1e
...
cd rufio && go build -ldflags="-X main.version=v1.0.6.3" -o ~/.local/bin/rufio ./cmd/rufio
```
...
https://github.com/user-attachments/assets/8ee256f9-dbc5-483f-bbc5-c4324719fdd5

*Zoom on the mesh view — agents and their in-flight thoughts rendered as a live network.*

## See it work

Four agents on different machines, different vendors, converge on one decision via the substrate.

```bash
# Agent A (Claude Code) — proposes a hypothesis
$ rufio think --type=hypothesis --subject=customer:5821 \
    --content="churning, pattern matches last quarter" --scope=fleet

# Agent B (Cursor, different machine) — finds it, confirms with new evidence
$ rufio recall "customer 5821"
# → claude-code · hypothesis · churning, pattern matches last quarter
$ rufio confirm <thought-id> --evidence="usage data agrees"

# Agent C (Codex) — independently confirms
$ rufio confirm <thought-id> --evidence="downgrade signals match"

# Agent D (Gemini) — hits quorum on a fourth machine
$ rufio confirm <thought-id> --evidence="account read agrees"
$ rufio recall "customer 5821"
# → [PROMOTED] cursor + codex + gemini confirmed claude-code's hypothesis.
#   Now durable knowledge in learned/, replayable forever.
```

Four agents. Three vendors. Four machines. One conclusion — reached without an orchestrator, an SDK setup, or shared memory.

[See the full walkthrough](./docs/demo.md)
...
Single static binary built with `go build ./cmd/rufio`. Race-clean tests on macOS + Ubuntu CI. Python SDK in `python/` (install via `pip install git+https://github.com/d-mcmillan/rufio.git@v1.0.6.3#subdirectory=python`).
...
│   ├── demo.md              ← the live walkthrough, runnable
│   ├── roadmap.md           ← what's next
│   ├── glossary.md          ← terms of art
│   └── release-history/     ← archived release notes pre-v1.0.6
